...
import { GetDataDbRequest, GetSingleRecordRequest, AddSingleDbRequest, UpdateSingleDbRequest, DeleteRowDbRequest, AggregationRequest, TableExistsRequest, } from "@cladbe/postgres_manager/dist/models/requests";
...
const RESPONSE_TOPIC_DEFAULT = process.env.SQL_RPC_RESPONSE_TOPIC || "sql.rpc.responses";
...
function okRows(corr, rows) {
    ...
    const env = FBG.SqlRpc.ResponseEnvelope.createResponseEnvelope(b, b.createString(corr), true, FBG.SqlRpc.ErrorCode.NONE, 0, FBG.SqlRpc.RpcResponse.RowsJson, rowsObj);
    b.finish(env);
    return Buffer.from(b.asUint8Array());
}
function okRow(corr, row) {
    const b = new flatbuffers.Builder(512);
    const rowStr = FBG.SqlRpc.RowJson.createRowJson(b, b.createString(JSON.stringify(row)));
    const env = FBG.SqlRpc.ResponseEnvelope.createResponseEnvelope(b, b.createString(corr), true, FBG.SqlRpc.ErrorCode.NONE, 0, FBG.SqlRpc.RpcResponse.RowJson, rowStr);
    b.finish(env);
    return Buffer.from(b.asUint8Array());
}
function okBool(corr, val) {
    const b = new flatbuffers.Builder(64);
    const boolRes = FBG.SqlRpc.BoolRes.createBoolRes(b, val);
    const env = FBG.SqlRpc.ResponseEnvelope.createResponseEnvelope(b, b.createString(corr), true, FBG.SqlRpc.ErrorCode.NONE, 0, FBG.SqlRpc.RpcResponse.BoolRes, boolRes);
    b.finish(env);
    return Buffer.from(b.asUint8Array());
}
function err(corr, code, message) {
    const b = new flatbuffers.Builder(256);
    const env = FBG.SqlRpc.ResponseEnvelope.createResponseEnvelope(b, b.createString(corr), false, code, b.createString(message), 0, 0);
    b.finish(env);
    return Buffer.from(b.asUint8Array());
}
// ----- helpers -----
function collect(m, base) {
...
function readCursorValue(ce) {
    ...
    if (vt === V.TimestampValue) {
        const v = new FBG.SqlSchema.TimestampValue();
        ce.value(v);
        return Number(v.epoch());
    }
    return undefined;
}
// ----- request handler -----
async function handleEnv(envBytes) {
    ...
    try {
        switch (req.method()) {
            case FBG.SqlRpc.RpcMethod.GET_DATA: {
                const m = new FBG.SqlRpc.GetDataReq();
                req.payload(m);
                const order = [];
                for (let i = 0; i < m.orderLength(); i++) {
                    const ok = new FBG.SqlSchema.OrderKeySpec();
                    m.order(i, ok);
                ...
                const tsReq = new GetDataDbRequest(m.tableName(), m.companyId(), undefined, filters, m.limit() || undefined, m.offset() || undefined, order, cursor, strictAfter);
                const rows = await pg.getData(tsReq);
                kafka.produceSafe(replyTopic, corr, okRows(corr, rows));
                break;
            }
            case FBG.SqlRpc.RpcMethod.GET_SINGLE: {
                const m = new FBG.SqlRpc.GetSingleReq();
                req.payload(m);
                const tsReq = new GetSingleRecordRequest(m.tableName(), m.companyId(), m.primaryKeyColumn(), m.primaryId());
                const row = await pg.getData(tsReq);
                kafka.produceSafe(replyTopic, corr, okRow(corr, row));
                break;
            }
            case FBG.SqlRpc.RpcMethod.ADD_SINGLE: {
                const m = new FBG.SqlRpc.AddSingleReq();
                req.payload(m);
                const tsReq = new AddSingleDbRequest(m.tableName(), m.companyId(), m.primaryKeyColumn(), JSON.parse(m.rowJson()));
                const rows = await pg.editData(tsReq);
                kafka.produceSafe(replyTopic, corr, okRows(corr, rows));
                break;
            }
            case FBG.SqlRpc.RpcMethod.UPDATE_SINGLE: {
                const m = new FBG.SqlRpc.UpdateSingleReq();
                req.payload(m);
                const tsReq = new UpdateSingleDbRequest(m.tableName(), m.companyId(), JSON.parse(m.updatesJson()), m.primaryKeyColumn(), m.primaryId());
                const rows = await pg.editData(tsReq);
                kafka.produceSafe(replyTopic, corr, okRows(corr, rows));
                break;
            }
            case FBG.SqlRpc.RpcMethod.DELETE_ROW: {
                const m = new FBG.SqlRpc.DeleteRowReq();
                req.payload(m);
            ...
            case FBG.SqlRpc.RpcMethod.TABLE_EXISTS: {
                const m = new FBG.SqlRpc.TableExistsReq();
                req.payload(m);
            ...
            case FBG.SqlRpc.RpcMethod.RUN_AGGREGATION: {
                const m = new FBG.SqlRpc.RunAggregationReq();
                req.payload(m);
                const wrapper = m.wrapper();
                const filters = wrapper ? [decodeWrapper(wrapper)] : undefined;
        ...
        kafka.produceSafe(replyTopic, corr, err(corr, FBG.SqlRpc.ErrorCode.INTERNAL, e?.message || "internal"));
    }
}
// ---- lifecycle ----
export async function startRpcWorker() {
    kafka.setHandler((msg) => {
        if (!msg.value)
            return;
        void handleEnv(new Uint8Array(msg.value));
    });
    await kafka.start();
