...
function encodeKeyValueVec(b, obj) {
...
function encodeTableOptions(b, obj) {
    if (!obj || !Object.keys(obj).length)
        return 0;
    const entries = [];
    for (const [k, v] of Object.entries(obj)) {
...
function encodeColumn(b, c) {
    ...
    if (customOff)
        sc.SqlSchema.TableColumn.addCustomOptions(b, customOff);
    return sc.SqlSchema.TableColumn.endTableColumn(b);
}
export function encodeTableDefinition(b, td) {
    ...
    if (commentOff)
        sc.SqlSchema.TableDefinition.addComment(b, commentOff);
    if (optsOff)
        sc.SqlSchema.TableDefinition.addTableOptions(b, optsOff);
